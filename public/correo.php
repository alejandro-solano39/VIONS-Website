...
$mail->Password = '123$Vions'; // Usa comillas simples o escapa el carácter $
$mail->setFrom('info@vions.com.mx', 'Mr. Drago');

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['email'], $_POST['name'])) {
    // Tomar el correo del destinatario del formulario
    $recipientEmail = $_POST['email'];
    $recipientName = $_POST['name'];

    // Validar el correo electrónico
    if (filter_var($recipientEmail, FILTER_VALIDATE_EMAIL)) {
        $mail->addAddress($recipientEmail, $recipientName); // Usar el correo del formulario
    } else {
