...
$activo = isset($artista['activo']) ? $artista['activo'] : 0;
?>
...
<body class="sb-nav-fixed">
            ...
            <main>
                <div class="container my-4">
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4">
                        <h1>Lista de Portafolios de Artistas525</h1>
                        <a href="form-portfolio.php" class="btn btn-success btn-lg d-flex align-items-center gap-2">
                    ...
                    <div class="card shadow-sm">
                        <div class="card-header">
                            <i class="fas fa-table me-1"></i> Portafolios
                        </div>
                                            ...
                                            <tr>
                                                <td>
                                                    <?php if (!empty($artista['miniatura'])): ?>
                                                        <img src="../<?php echo htmlspecialchars($artista['miniatura']); ?>" alt="Miniatura" width="60" height="60" class="rounded">
                                                    <?php else: ?>
                                                        <span class="text-muted">Sin miniatura</span>
                                                    <?php endif; ?>
                                                </td>
                                                <td><?php echo htmlspecialchars($artista['nombre']); ?></td>
                                                <td><?php echo htmlspecialchars($artista['rol']); ?></td>
                                                <td><?php echo htmlspecialchars($artista['categoria_nombre']); ?></td>
                                                <td><?php echo htmlspecialchars($artista['contacto']); ?></td>
                                                <td>
                                                    <a href="form-portfolio.php?id=<?php echo $artista['id']; ?>" class="btn btn-primary btn-sm">
                                                        <i class="fas fa-edit"></i> Editar
                                                    </a>
                                                    <button class="btn btn-danger btn-sm delete-btn" data-id="<?php echo $artista['id']; ?>">
                                                        <i class="fas fa-trash-alt"></i> Eliminar
                                                    </button>
                                                    <?php if ($artista['activo']): ?>
                                                        <button class="btn btn-success btn-sm toggle-btn" data-id="<?php echo $artista['id']; ?>">
                                                            <i class="fas fa-check-circle"></i> Activo
                                                        </button>
                                                    <?php else: ?>
                                                        <button class="btn btn-secondary btn-sm toggle-btn" data-id="<?php echo $artista['id']; ?>">
    ...
    <script>
        ...
        $(document).on('click', '.delete-btn', function() {
            const button = $(this); // Botón que se clickeó
            const idToDelete = button.data('id'); // ID del portafolio a eliminar
            const rowToDelete = button.closest('tr'); // Fila de la tabla asociada al botón
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '../actions/delete-portfolio.php', // Ruta al archivo PHP
                        type: 'POST',
                        data: {
                            id: idToDelete
                        }, // ID enviado al servidor
                        dataType: 'json',
                        success: function(response) {
                            if (response.success) {
                                // Eliminar fila y mostrar mensaje de éxito
                                rowToDelete.remove();
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Eliminado',
        ...
        $(document).on('click', '.toggle-btn', function() {
            ...
            $.ajax({
                url: '/admin/actions/toggle-portfolio.php',
                type: 'POST',
                data: {
                    id: id
                },
